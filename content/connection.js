...
Salesforce.com AJAX Connector 9.0
...
sforce.internal.Debug = function() {
    ...
    this.println = function(s, type) {
        ...
        if (this.win === null || !this.win.document) {
            this.output = null;
            this.win = window.open('/soap/ajax/9.0/debugshell.html', '',
    ...
    this.logApex = function(response) {
        ...
        var msg = start === -1 ? response : response.substring(start);
        msg = end === -1 ? msg : msg.substring(0, end);
...
sforce.XmlReader.prototype.getTextValue = function(node) {
    ...
    for (var i = 0; i < children.length; i++) {
        if (children[i].nodeType == 3) {
            sb.append(children[i].nodeValue);
        } else {
...
sforce.Connection = function() {
    ...
    this.serverUrl = "/services/Soap/u/9.0";
};
...
sforce.Connection.prototype.remoteFunction = function(args) {
    ...
    if (sforce.debug.trace) {
        sforce.debug.log("Open connection ...");
    }
        ...
        if (request.readyState == 4) {
            if (request.status == 200) {
                if (args.mimeType == "text/plain") {
                    ...
                    args.onSuccess(request.responseXML.documentElement);
                } else {
